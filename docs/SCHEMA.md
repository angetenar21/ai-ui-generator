...
| data | `string` | No | Object-array format: [{month: "Jan", revenue: 42000, expenses: 38000}, ...] |
| xKey | `string` | No |  |
| seriesKeys | `string` | No |  |
...
| data | `string` | No | Data as an array of record objects, e.g.
...
| xKey | `string` | No | The key in each data object that provides x-axis category labels.
Required when using the object-array `data` format. |
| seriesKeys | `string` | No | Explicit series keys to extract from `data` objects.
...
| className | `string` | No | Optional CSS class names |
| data | `string` | No | Object-array format: [{month: "Jan", sales: 4200, returns: 380}, ...] |
| xKey | `string` | No | Key in data objects used as x-axis categories (required with data) |
| seriesKeys | `string` | No | Explicit keys to extract as series; defaults to all numeric keys except xKey |
...
Combined bar and line chart with dual Y-axes for comparing different scales
...
Multiple line series chart for comparing trends across categories
...
|----------|------|----------|-------------|
| title | `string` | No |  |
| position | `'left' | 'right' | 'top' | 'bottom'` | No |  |
| defaultOpen | `boolean` | No |  |
| size | `'small' | 'medium' | 'large' | 'full'` | No |  |
| variant | `'default' | 'elevated' | 'overlay'` | No |  |
| backdrop | `boolean` | No |  |
...
A toggle switch or button used to change the application theme between light and dark mode.
...
| size | `'small' | 'medium' | 'large'` | No | Size of the toggle |
| variant | `'icon' | 'button' | 'switch'` | No | Display format |
...
Interactive map powered by react-leaflet + OpenStreetMap. Supports markers with popups, polylines, polygons, circles, multiple tile styles (streets, dark, satellite, light), map legend, title/subtitle header, and all common geospatial UI patterns. Completely free with no API key required.
...
| center | `number` | No |  |
| zoom | `number` | No |  |
| tileLayer | `'streets' | 'dark' | 'satellite' | 'light'` | No |  |
| markers | `array` | No |  |
| polylines | `array` | No |  |
| polygons | `array` | No |  |
| circles | `array` | No |  |
...
| subtitle | `string` | No |  |
| legend | `array` | No |  |
| showZoomControl | `boolean` | No |  |
| scrollWheelZoom | `boolean` | No |  |
| height | `string` | No |  |
| rounded | `boolean` | No |  |
| shadow | `boolean` | No |  |
| className | `string` | No |  |

...
const DataTable: React.FC<DataTableProps> = ({
  ...
  const renderCellValue = (value: any): React.ReactNode => {
    ...
    if (typeof value === 'object') {
      ...
      if (value.name || value.type) {
        // Flat objects generated by AI won't pass props down unless they are nested in templateProps
        const spec = (value.templateProps || value.props)
          ? value
          : { name: value.name || value.type, templateProps: { ...value } };
  ...
  return (
    <div className="w-full max-w-full bg-transparent border border-white/20 dark:border-white/[0.08] rounded-2xl overflow-hidden my-2">
      {title && (
        <div className="px-6 py-4 border-b border-white/15 dark:border-white/[0.07]">
      ...
      {searchable && (
        <div className="px-6 py-3 border-b border-white/15 dark:border-white/[0.06]">
          <input
            type="text"
            placeholder="Search table..."
            value={searchQuery}
            onChange={(e) => setSearchQuery(e.target.value)}
            className="w-full px-3.5 py-2 text-sm border border-white/25 dark:border-white/[0.08] rounded-xl
                     bg-white/30 dark:bg-white/[0.05] text-zinc-900 dark:text-zinc-100 placeholder-zinc-400
                     focus:outline-none focus:ring-2 focus:ring-orange-500/40 focus:border-orange-400/50 transition-all duration-300"
          />
        </div>
      )}
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
          <thead className="sticky top-0 z-10">
            <tr className="bg-gradient-to-r from-orange-500/[0.07] to-pink-500/[0.04] dark:from-orange-500/[0.12] dark:to-pink-500/[0.06] border-b-2 border-orange-200/30 dark:border-orange-500/15">
              {normalizedColumns.map((column, index) => (
                <th
                  key={index}
                  onClick={() => handleSort(index)}
                  className={`px-5 py-3.5 text-left text-[10px] font-bold text-orange-600/70 dark:text-orange-400/70 uppercase tracking-[0.12em]
                           whitespace-nowrap
                           ${sortable ? 'cursor-pointer hover:text-orange-700 dark:hover:text-orange-300 select-none transition-colors duration-200' : ''}`}
                >
            ...
            {displayRows.map((row, rowIndex) => (
              <tr
                ...
                className={`
                  ...
                  ${rowIndex % 2 === 0 ? 'bg-transparent' : 'bg-white/[0.08] dark:bg-white/[0.03]'}
                `}
              >
                {row.map((cell, cellIndex) => (
                  <td
                    ...
                    className="px-5 py-3.5 text-sm text-zinc-700 dark:text-zinc-300
                             border-b border-white/15 dark:border-white/[0.05]
                             max-w-[240px]"
      ...
      {displayRows.length > 0 && (
        <div className="px-6 py-3 border-t border-white/15 dark:border-white/[0.06] bg-white/10 dark:bg-white/[0.02] flex items-center justify-between">
